...
def parse_json_file():
    ...
    for f in glob.glob('training_data/*task*.json'):
        ...
        for x in npdf:
            ...
            for y in x_json[0]['data']:
                # print(y)
                y_words = y['text'].split(' ')
                if 'entity' in y.keys():
                    for word in y_words:
                        word = word.replace(':', '').replace(',', '').replace('.', '').replace('?', '').replace(';', '').replace(' ', '')
                        tags.append(y['entity'])
                        sent.append(word)
                else:
                    for word in y_words:
                        word = word.replace(':', '').replace(',', '').replace('.', '').replace('?', '').replace(';', '').replace(' ', '')
            ...
            # print([(x,y) for x, y in zip(tags, sent)])
            tags = []
            sent = []
            # print('\n\n\n')

    return sent_lst, tags_lst

sentences, labels = parse_json_file()
# print([(x,y) for x, y in zip(sentences[0], labels[0])])
...
for _ in trange(epochs, desc="Epoch"):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        outputs = model(torch.tensor(b_input_ids).to(torch.int64), token_type_ids=None, attention_mask=b_input_mask, labels=b_labels)
        # get the loss
        loss = outputs[0]
